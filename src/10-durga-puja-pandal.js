...
export function filterPandalsByZone(container, zone) {
  ...
  for(let i=0;i<elements.length;i++){
    ...
    if(elements[i].dataset.zone === zone)
...
export function getPandalsByBudgetRange(container, min, max) {
  ...
  for(let i=0;i<elements.length;i++){
    ...
    const budget = Number(elements[i].dataset.budget)
...
export function sortPandalsByRating(container) {
  ...
  const elements = Array.from(container.querySelectorAll(".pandal"))

  elements.sort((a,b) => Number(b.dataset.rating) - Number(a.dataset.rating))
